...
public sealed class GifConverterTest
{
    ...
    private static string FolderName
        => Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly()?.Location) ?? "", typeof(GifConverterTest).ToString().Split('.').Last());
        ...
        bool isFilenameEmpty)
    {
        ...
        string filename = isFilenameEmpty ? string.Empty : MethodBase.GetCurrentMethod()?.Name + ".gif";

        GifConverter converter = new();
    ...
    public void TextToImg_IncorrectFoldername_ReturnsException()
    {
        ...
        string filename = MethodBase.GetCurrentMethod()?.Name + ".gif";
