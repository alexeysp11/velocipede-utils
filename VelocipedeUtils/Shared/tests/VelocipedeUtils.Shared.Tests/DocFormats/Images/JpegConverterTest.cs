...
public sealed class JpegConverterTest
{
    ...
    private static string FolderName
        => Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly()?.Location) ?? "", typeof(JpegConverterTest).ToString().Split('.').Last());
        ...
        bool isFilenameEmpty)
    {
        ...
        string filename = isFilenameEmpty ? string.Empty : MethodBase.GetCurrentMethod()?.Name + ".jpeg";

        JpegConverter converter = new();
    ...
    public void TextToImg_IncorrectFoldername_ReturnsException()
    {
        ...
        string filename = MethodBase.GetCurrentMethod()?.Name + ".jpeg";
