...
/// </summary>
public sealed class SqlServerDbConnection : BaseVelocipedeDbConnection, IVelocipedeDbConnection
{
    /// <inheritdoc/>
    ...
    /// <summary>
    /// Creates an instance of <see cref="SqlServerDbConnection"/> by connection string.
    /// </summary>
    /// <param name="connectionString">Specified connection string.</param>
    public SqlServerDbConnection(string? connectionString = null)
    {
...
WHERE s.type = 'TR' and object_name(parent_obj) = @TableName";
    ...
    /// <returns>Instance of <see cref="SqlServerDbConnection"/>.</returns>
    private SqlServerDbConnection OpenDb(string? connectionString)
    {
        if (string.IsNullOrEmpty(connectionString))
            throw new InvalidOperationException(ErrorMessageConstants.ConnectionStringShouldNotBeNullOrEmpty);
